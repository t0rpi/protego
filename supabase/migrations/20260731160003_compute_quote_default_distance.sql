-- M7 QA fix: use pricing_config.default_distance_km as a fallback when
-- the client leaves Protect Ride's km field blank, instead of silently
-- charging base-fare-only. The estimate is flagged as such in the
-- returned line (label 'distance_estimated' vs 'distance') so the
-- client-facing quote never presents a guess as a measured distance.
