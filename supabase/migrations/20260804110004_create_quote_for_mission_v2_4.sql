-- Pass accompany_inside_minutes (replaces the dropped boolean) into
-- compute_quote(). m.distance_km was already passed unconditionally
-- for every service, so Escort/Hourly's new km surcharge just starts
-- working once the client populates it -- no other change needed here.
